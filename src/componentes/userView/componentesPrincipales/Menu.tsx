...
const Menu = () => {
    ...
    const { setVentana } = useVentana() //Ventana
    const [selectedItem, setSelectedItem] = useState<string | null>('Academias'); //Item seleccionado
    ...
    const iconoAcademias = (<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="512.000000pt" height="512.000000pt" viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet"
        className={`
        ${styles.iconItem} 
        ${selectedItem === 'Academias' ? styles.activeIcon : styles.inactiveIcon}`
        }>
    ...
    const iconoMensajes = (<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="512.000000pt" height="512.000000pt" viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet"
        className={`
            ${styles.iconItem} 
            ${selectedItem === 'Mensajes' ? styles.activeIcon : styles.inactiveIcon}`
        }>
        <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)" stroke="none">
    ...
    const iconoActividad = (<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="512.000000pt" height="512.000000pt" viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet"
        className={`
        ${styles.iconItem}
        ${selectedItem === 'Actividad' ? styles.activeIcon : styles.inactiveIcon}`
        }>
        <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)" stroke="none">
    ...
    const iconoCalendario = (<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="512.000000pt" height="512.000000pt" viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet"
        className={`
            ${styles.iconItem}
            ${selectedItem === 'Calendario' ? styles.activeIcon : styles.inactiveIcon}`
        }>
        <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)" stroke="none">
...
const getMenuContainerClass = () => {
    const baseClass = `${styles.menuContainer}`;

    if (screenSize === 'large') {
        if (menuType === 'extendido') {
            return `${baseClass} ${styles.menuContainerExtendido}`;
        }
        if (menuType === 'reducido') {
            return `${baseClass} ${styles.menuContainerReducido}`;
        }
        if (menuType === 'oculto') {
            return `${baseClass} ${styles.menuContainerOculto}`;
        }
    }
...
const getItemTextClass = () => {
    const baseClass = `${styles.textItem}`;

    if (menuType === 'extendido') {
        if (screenSize === 'large') {
            return `${baseClass} ${styles.textItemVisible}`;
        }
        if (screenSize === 'medium') {
            return `${baseClass} ${styles.textItemVisible}`;
        }
    ...
    if (menuType === 'reducido') {
        if (screenSize === 'large') {
            return `${baseClass} ${styles.textItemHidden}`;
        }
        if (screenSize === 'medium') {
            return `${baseClass} ${styles.textItemHidden}`;
        }
            ...
            <nav className={styles.menu}>
                <ul className={styles.menuList}>
                    ...
                    <li 
                        className={`
                        ${styles.menuItem}
                        ${selectedItem === 'Academias' ? styles.menuItemSelected : ''}`
                        } onClick={() => handleSelect('Academias')}
                    >
                        {iconoAcademias}
                        <span className={getItemTextClass()}>Academia</span>
                    </li>

                    <li 
                        className={
                        `${styles.menuItem}
                        ${selectedItem === 'Mensajes' ? styles.menuItemSelected : ''}`
                        } onClick={() => handleSelect('Mensajes')}
                    >
                        {iconoMensajes}
                        <span className={getItemTextClass()}>Mensajes</span>
                    </li>

                    <li 
                        className={`
                        ${styles.menuItem}
                        ${selectedItem === 'Actividad' ? styles.menuItemSelected : ''}`}
                        onClick={() => handleSelect('Actividad')}
                    >
                        {iconoActividad}
                        <span className={getItemTextClass()}>Actividad</span>
                    </li>

                    <li
                        className={`
                        ${styles.menuItem}
                        ${selectedItem === 'Calendario' ? styles.menuItemSelected : ''}`}
                        onClick={() => handleSelect('Calendario')}
                    >
                        {iconoCalendario}
                        <span className={getItemTextClass()}>Calendario</span>
                    </li>
                </ul>
            </nav>
        </div >
